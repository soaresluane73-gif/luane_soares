<div align="center">
  <!-- Banner azul com efeito de ondas -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0066cc&height=220&section=header&text=Olá,%20sou%20a%20Luane%20Soares!&fontSize=40&fontColor=ffffff&animation=twinkling" width="100%" />

  <br />

  <h3>💙 Desenvolvedora Full Stack em Formação | Especialista em Mobile & Backend 💙</h3>

  <p>
    </a>
  </p>

  <br />
</div>

---

## 🔹 Sobre Mim

- 🎓 **Curso:** Análise e Desenvolvimento de Sistemas (*Projeto Mobile 2026*).
- 📱 **Projeto Atual:** Aplicação móvel de **Achados e Perdidos** para o campus universitário.
- ⚙️ **Foco de Estudo:** Arquitetura de APIs no backend com NestJS e interfaces dinâmicas no React Native.
- 🌐 **Idioma da Documentação:** Padrão internacional com código limpo e commits semânticos.
- 💙 **Paixão:** Criar soluções funcionais, elegantes e resolver problemas de sincronização no Git!

---

## 🛠️ Tecnologias & Domínios

### 📱 Frontend & Mobile
<p>
  <img src="https://img.shields.io/badge/React_Native-0088CC?style=for-the-badge&logo=react&logoColor=white" />
  <img src="https://img.shields.io/badge/Expo-005580?style=for-the-badge&logo=expo&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-005F9E?style=for-the-badge&logo=javascript&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white" />
</p>

### ⚙️ Backend & Base de Dados
<p>
  <img src="https://img.shields.io/badge/NestJS-185ADB?style=for-the-badge&logo=nestjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-1C7293?style=for-the-badge&logo=nodedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Prisma-063970?style=for-the-badge&logo=prisma&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-006699?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_API-003366?style=for-the-badge&logo=fastapi&logoColor=white" />
</p>

### 🛠️ Ferramentas & Fluxo de Trabalho
<p>
  <img src="https://img.shields.io/badge/Git-00529B?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-0A192F?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-0db7ed?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Figma-0284C7?style=for-the-badge&logo=figma&logoColor=white" />
</p>

---

## 🚀 Projetos Principais

| Projeto | Descrição | Tecnologias Chave | Estado |
| :--- | :--- | :--- | :---: |
| **🔍 App Achados & Perdidos** | Sistema para publicar, filtrar e recuperar objetos perdidos na universidade. | `React Native`, `NestJS`, `Prisma` | 🟡 Em Progresso |
| **⚡ Listing API Service** | Endpoint de consulta com busca insensível a maiúsculas, paginação e segurança de dados do autor. | `NestJS`, `TypeScript`, `Prisma` | 🟢 Concluído |
| **📱 Task Manager App** | Aplicação móvel de organização de tarefas diárias com persistência local. | `Expo`, `TypeScript` | 🟢 Concluído |

---

## 📈 Estatísticas do GitHub (Tema Azul / Tokyonight)

<div align="center">
  <br />
  <!-- Visão geral de commits e atividade -->
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=soaresluane73-gif&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true" />
  
  <!-- Linguagens mais utilizadas -->
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=soaresluane73-gif&layout=compact&theme=tokyonight" />
  
  <br /><br />

  <!-- Sequência de dias ativos (Streak) -->
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=soaresluane73-gif&theme=tokyonight" />
</div>

---

## 🤝 Conecta-te Comigo

<div align="center">

  <a href="https://www.linkedin.com/in/seu-perfil" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:seu-email@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-0059B3?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://instagram.com/seu-perfil" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-0284C7?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>

  <br /><br />

  <!-- Contador de Visitas no Perfil -->
  <img src="https://profile-counter.glitch.me/soaresluane73-gif/count.svg" alt="Contador de Visitas" />
</div>

  <br /><br />

  <!-- Contador de Visitas no Perfil -->
  <img src="https://profile-counter.glitch.me/soaresluane73-gif/count.svg" alt="Contador de Visitas" />
</div>
